import java.io.*;
import java.math.BigDecimal;
import java.math.BigInteger;
import java.nio.charset.StandardCharsets;
import java.util.*;
import java.net.*;

public class AESClient {
    public static final int[] sbox = {
        };
    public static final int[] invsbox = {
        };

    public static final int[][] Rijndaelmatrix={{2,3,1,1},{1,2,3,1},{1,1,2,3},{3,1,1,2}};
    public static final int[][] invMixColumnMatrix={{14,11,13,9},{9,14,11,13},{13,9,14,11},{11,13,9,14}};


    public static final int [] multiplyBy9={
        };

    public static final int [] multiplyBy11 = {
        };

    public static final int [] multiplyBy13={
        };

    public static final int [] multiplyBy14={
        };

    public static int [][] roundKeys = new int[44][4];

    public static final int [] rconst = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

    public static String hexToUtf(String hex) {
        if (hex.length() % 2 != 0) {
            throw new IllegalArgumentException("Hex string must have an even length");
        }

        byte[] bytes = new byte[hex.length() / 2];
        for (int i = 0; i < hex.length(); i += 2) {
            bytes[i / 2] = (byte) ((Character.digit(hex.charAt(i), 16) << 4)
                                 + Character.digit(hex.charAt(i + 1), 16));
        }
        return new String(bytes, StandardCharsets.UTF_8);
    }


    private static String binToHex(String bin) {
        
        BigInteger b = new BigInteger(bin, 2);
        String ciphertext = b.toString(16);
        
        return ciphertext;
    }
    
    private static String utfToBin(String utf) {
        
        byte[] bytes = null;
        try {
            bytes = utf.getBytes("utf-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        
        String bin = "";
        for (int i = 0; i < bytes.length; i++) {
             int value = bytes[i];
             for (int j = 0; j < 8; j++)
             {
                bin += ((value & 128) == 0 ? 0 : 1);
                value <<= 1;
             }
        }
        return bin;
    }

    private static int [] rotWord(int [] word){
        int [] newList = new int[4];
        newList[3] = word[0];
        for(int i=0; i<word.length-1;i++){
            newList[i] = word[i+1];
        }
        return newList;
    }

    private static int [] subWord(int [] word){
        for(int i=0; i<word.length;i++){
            word[i] = sbox[word[i]];

        }
        return word;
    }

    private static void keyExpansion(int[][] key){
        for(int i=0; i<4; i++){
            roundKeys[i] = getColumn(key, i);
        }
        for(int i=0;i<4*(11);i++){
            if(i<4){
                roundKeys[i] = getColumn(key, i);
            }
            else if(i>=4 && i%4==0){
                int [] temp = subWord(rotWord(roundKeys[i-1]));
                for(int j=0; j<4; j++) {
                    temp[j] = temp[j]^roundKeys[i - 4][j];
                }
                temp[0] = temp[0]^rconst[(i-4)/4];
                roundKeys[i] = temp;
            }
            else{
                for(int j=0; j<4; j++){
                    roundKeys[i][j] = roundKeys[i-4][j] ^ roundKeys[i-1][j];
                }
            }
        }
    }

    private static int[] getColumn(int[][] matrix, int column){
        int[] col = new int[4];
        for(int row = 0; row < matrix.length; row++) {
            col[row] = matrix[row][column];
            }
        return col;
    }

    private static int [][] mixColumns(int [][]state,boolean dec){
        int temp[][]=new int[4][4];
        if(!dec){
            for(int rowRijndael=0;rowRijndael<4;rowRijndael++){
                for(int stateCol=0;stateCol<4;stateCol++){
                    for(int cell=0;cell<4;cell++){
                        if(Rijndaelmatrix[rowRijndael][cell]==1){
                            temp[rowRijndael][stateCol]^=state[cell][stateCol];

                        }else if(Rijndaelmatrix[rowRijndael][cell]==2){
                            int shiftedNum=multiplyBy2(state[cell][stateCol]);
                            temp[rowRijndael][stateCol]^=shiftedNum;
                        }else{
                            int shiftedNum=multiplyBy2(state[cell][stateCol]);
                            temp[rowRijndael][stateCol]^=shiftedNum^state[cell][stateCol];
                        }
                    }
                }
            }
            return temp;
        }else{
            for(int invMixRow=0;invMixRow<4;invMixRow++){
                for(int stateCol=0;stateCol<4;stateCol++){
                    for(int cell=0;cell<4;cell++){
                        
                        if(invMixColumnMatrix[invMixRow][cell]==9){
                            temp[invMixRow][stateCol]^=multiplyBy9[state[cell][stateCol]];
                        }else if(invMixColumnMatrix[invMixRow][cell]==11){
                            temp[invMixRow][stateCol]^=multiplyBy11[state[cell][stateCol]];
                        }else if(invMixColumnMatrix[invMixRow][cell]==13){
                            temp[invMixRow][stateCol]^=multiplyBy13[state[cell][stateCol]];
                        }else{
                            temp[invMixRow][stateCol]^=multiplyBy14[state[cell][stateCol]];
                        }
                    }
                }
            }
            return temp;
        }
    }

    private static int multiplyBy2(int num){
        int shiftedNum=(num<<1)&255;
        
        if((num&(1<<7))==(1<<7)){
            shiftedNum^=0x1B;
            
        }
        
        return shiftedNum;
    }

    private static int[][] subBytes(int[][] state,boolean dec){
        for(int i=0;i<4;i++){
            for(int j=0;j<4;j++){
                if(dec){
                state[i][j]=invsbox[state[i][j]];
                }
                else{
                state[i][j]=sbox[state[i][j]];
            }}
        }
        return state;
    }

    private static int[][] encipher(int[][]matState, int[][] key, boolean dec){
        if(!dec){
            matState = addRoundKey(matState, 0);
            for(int i=0; i<10; i++){
                matState=subBytes(matState, dec);
                matState=shiftRows(matState,dec);
                if(i!=9) {
                    matState = mixColumns(matState, dec);
                }
                matState =addRoundKey(matState, i+1);
            }
        }
        else{
            matState =addRoundKey(matState, 10);
            matState=shiftRows(matState,dec);
            matState=subBytes(matState, dec);
            for(int i=9; i>0; i--){
                matState =addRoundKey(matState, i);
                matState = mixColumns(matState, dec);
                matState=shiftRows(matState,dec);
                matState=subBytes(matState, dec);
                System.out.println("Round "+(10-i)+": "+printFile(matState));
            }
            matState =addRoundKey(matState, 0);
            System.out.println("Round 10: "+printFile(matState));
        }
        return matState;
    }

    private static int[][] shiftRows(int[][] state,boolean dec){
        if(!dec) {
            int temp[]=new int[4];
            for(int i=0;i<4;i++){
                temp[(i-1+4)%4]=state[1][i];
            }
            state[1]=temp.clone();

            for(int i=0;i<4;i++){
                temp[(i-2+4)%4]=state[2][i];
            }
            state[2]=temp.clone();

            for(int i=0;i<4;i++){
                temp[(i-3+4)%4]=state[3][i];
            }
            state[3]=temp.clone();
            return state;
        }
        else {
            int temp[]=new int[4];
            for(int i=0;i<4;i++){
                temp[(i+1)%4]=state[1][i];
            }
            state[1]=temp.clone();

            for(int i=0;i<4;i++){
                temp[(i+2)%4]=state[2][i];
            }
            state[2]=temp.clone();

            for(int i=0;i<4;i++){
                temp[(i+3)%4]=state[3][i];
            }
            state[3]=temp.clone();
            return state;
        }
    }

    private static int[][] addRoundKey(int[][] state, int index){
        int [][] roundKey = getRoundKeyMatrix(index);
        for(int i=0;i<4;i++){
            for(int j=0;j<4;j++){
                state[i][j]=state[i][j] ^ roundKey[i][j];
            }
        }
        return state;
    }

    private static int[][] getRoundKeyMatrix(int index){
        int[][] temp = new int[4][4];
        for(int i=0; i<4;i++){
            int ind = index*4+i;
            for(int j=0; j<4; j++){
                temp[j][i] = roundKeys[ind][j];
            }
        }
        return temp;
    }

    private static String printFile(int[][] matState){
        String context = "";
        for(int i=0;i<4;i++){
            for(int j=0;j<4;j++){
                String val = Integer.toHexString(matState[j][i]);
                if (val.length() == 1) {
                    val = "0" + val;
                }
                context += val;
            }
        }
        return context;
    }
    
    
    public static void main(String[] args) {
        try {
            Socket socket = new Socket("localhost", 1234);
            BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            PrintWriter out = new PrintWriter(socket.getOutputStream(), true);
            System.out.println("Waiting for server...");
            
            int [][] matState= new int[4][4];
            int [][] key = new int[4][4];    
            String cipherText;
            while ((cipherText = in.readLine()) != null) {
                System.out.println("Encrypted message recieved: " + cipherText);

                Scanner sc = new Scanner(System.in);
                String keyData = "Sample key";
                
                while(keyData.length()<16){
                    keyData+=keyData;
                }
                if(keyData.length()>16) keyData=keyData.substring(0, 16);

                keyData = binToHex(utfToBin(keyData));
                for(int i=0;i<4;i++){
                    for(int j=0;j<4;j++){
                        int start=2*(4*i+j);
                        String keyHex=keyData.substring(start, start+2);
                        int keyDecimal=Integer.parseInt(keyHex,16);
                        key[j][i]=keyDecimal;
                    }
                }
                keyExpansion(key);

                String plainText="";
                for(int k=0; k<cipherText.length()/32; k++){
                    String part=cipherText.substring(k*32, (k+1)*32);
                    for(int i=0;i<4;i++){
                        for(int j=0;j<4;j++){
                            int start=2*(4*i+j);
                            String hex=part.substring(start, start+2);
                            int decimal=Integer.parseInt(hex,16);
                            matState[j][i]=decimal;
                        }
                    }
                    matState=encipher(matState,key,true);
                    plainText += printFile(matState);
                }                
                System.out.println("Decrypted text: " + hexToUtf(plainText));
            }
            in.close();
            out.close();
            socket.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
}