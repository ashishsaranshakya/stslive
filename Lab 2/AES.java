...
public class AES {
	...
	public static void main(String [] args) {
		...
		data=data.trim();
		data=binToHex(utfToBin(data));
		while(data.length()%32!=0) data+="0";

		String cipherText="";
		for(int k=0; k<data.length()/32; k++){
			String part=data.substring(k*32, (k+1)*32);
			for(int i=0;i<4;i++){
				for(int j=0;j<4;j++){
					int start=2*(4*i+j);
			...
			matState=encipher(matState,key,false);
			cipherText += printFile(matState);
		}
		...
		System.out.println("Plaintext: "+hexToUtf(plainText));
	}
